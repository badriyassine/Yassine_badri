...
const projectImages = ["/images/hero/1.png", "/images/hero/2.png", "/images/hero/3.png"];
...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    } else clearInterval(slideshowRef.current);
    return () => clearInterval(slideshowRef.current);
  }, [slideshowActive]);

  // Hide slideshow on scroll
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="max-w-xl sm:max-w-2xl mt-6 z-10"
      >
        {/* Glass section */}
        <div className="mx-auto mb-6 rounded-2xl border border-[#ff734d] backdrop-blur-xl px-4 py-2 flex items-center gap-3 shadow-md inline-flex text-sm sm:text-base">
          <div className="w-2 h-2 rounded-full bg-[#ff734d]"></div>
          <h3 className="font-medium overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <a
            href="#projects"
            className="relative inline-flex px-6 sm:px-8 py-3 rounded-full bg-[#ff734d] border border-[#ff734d] text-white font-medium items-center gap-2 sm:gap-3 shadow-md group transition-transform duration-300 hover:scale-105 hover:shadow-xl"
          >
          ...
          <a
            ...
            className="inline-flex md:hidden relative px-6 py-3 rounded-full border border-[#ff734d] bg-[#05070869] font-medium text-[#ff734d] items-center gap-2 shadow-md overflow-hidden group hover:opacity-80 mt-2 sm:mt-0"
          >
            <FaGithub className="z-10" />
            <span className="relative z-10 text-sm">Check My GitHub</span>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="hidden md:inline-flex relative px-6 sm:px-8 py-3 rounded-full border border-[#ff734d] bg-[#05070869] font-medium text-[#ff734d] items-center gap-2 shadow-md overflow-hidden group hover:opacity-80"
          >
            <FaGithub className="z-10" />
            <span className="relative z-10 text-sm sm:text-base">Check My GitHub</span>
            <span className="absolute inset-0 rounded-xl border-2 border-transparent animate-[pulse_2s_infinite]"></span>
          </a>
        </div>
      </motion.div>
      ...
      {/* Scroll */}
      <motion.div
        className="absolute bottom-8 flex flex-col items-center text-gray-300 text-xs sm:text-sm z-10"
        animate={{ y: [0, -10, 0] }}
      ...
      {showOnScroll && (
        <button
          ...
          className="absolute bottom-8 left-4 sm:left-8 p-2 sm:p-3 rounded-full bg-[#ff734d] text-white shadow-md hover:opacity-80 transition-colors z-10 cursor-pointer"
        >
          {slideshowActive ? <FaStop size={16} /> : <FaPlay size={16} />}
        </button>
      )}
